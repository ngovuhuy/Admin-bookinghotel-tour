...
const PieChartHotel = ({ setPieChartHotel }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        let data;
        if (timeRange === "week") {
          data = await dashBoardService.getTopHotelOfSupplierInWeek();
          ...
          data = await dashBoardService.getTopHotelOfSupplierDateRange(
            startDate,
            endDate
          );
        }
        ...
        //   ],
        //   ...data.map((item) => [
        //     `${item.hotelName || item.name} | ${item.supplierName}`,
        //     item.orderCount,
        //   ]),
        // ];
        ...
        if (data && data.length > 0) {
          const formattedData = [
            [
              timeRange === "week"
                ? "Most Frequently Ordered Rooms (This Week)"
                : `Most Frequently Ordered Rooms (${startDate} to ${endDate})`,
              "Order Count",
            ],
            ...data.map((item) => [
          ...
          setChartData(formattedData);
          setPieChartHotel(formattedData);
          setHasData(true);
        } else {
          setChartData([]);
          //setPieChartData([]);
  ...
  }, [timeRange, startDate, endDate, setPieChartHotel]);
    ...
    const drawChart = () => {
      if (chartRef.current) {
        if (dateError) {
          ...
          const title =
            timeRange === "custom"
              ? `Most Frequently Ordered Hotels (${startDate} to ${endDate})`
              : `Most Frequently Ordered Hotels (${timeRange})`;
          const options = {
            title: title,
            pieHole: 0.4,
          };
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Most Frequently Ordered Hotels");
    XLSX.writeFile(wb, "MostFrequentlyOrderedHotels.xlsx");
  };
