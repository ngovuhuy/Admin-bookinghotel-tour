...
const PieChartTour = ({ setPieChartTour }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        let data;
        if (timeRange === "week") {
          data = await dashBoardService.getTopTourOfSupplierInWeek();
          ...
          data = await dashBoardService.getTopTourOfSupplierDateRange(
            startDate,
            endDate
          );
        }
        ...
        //   ],
        //   ...data.map((item) => [
        //     `${item.tourName || item.name} | ${item.supplierName}`,
        //     item.orderCount,
        //   ]),
        // ];
        ...
        if (data && data.length > 0) {
          const formattedData = [
            [
              timeRange === "week"
                ? "Most Frequently Ordered Tours (This Week)"
                : `Most Frequently Ordered Tours (${startDate} to ${endDate})`,
              "Order Count",
            ],
            ...data.map((item) => [
  ...
  }, [timeRange, startDate, endDate, setPieChartTour]);
